...
export const generateGoogleAppsScript = async (
  ...
  Context:
  The user wants to generate seating chart tables in the CURRENT active Google Doc.
  ...
  1. Open the active document using 'DocumentApp.getActiveDocument()'. Do NOT use 'openById'.
  2. For each item in the data:
     a. Append a Page Break (except for the very first one if the doc is empty).
     b. Append a Paragraph with the 'fileName' as a Heading 1 title.
     c. Create the Grid/Table:
        - CRITICAL: 'body.appendTable()' DOES NOT accept arguments. Do not pass (rows, cols).
        - Instead, append an empty table: 'var table = body.appendTable();'
        - Then, iterate ${config.rows} times to append rows using 'table.appendTableRow()'.
